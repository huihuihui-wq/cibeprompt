...
1. **Skill definition** ‚Ü?`~/.config/opencode/skills/cineprompt/SKILL.md`
2. **Slash command** ‚Ü?`~/.config/opencode/commands/cine.md`
...
```bash
# Unix
test -f ~/.config/opencode/skills/cineprompt/SKILL.md && echo "‚ú?SKILL.md installed"
test -f ~/.config/opencode/commands/cine.md && echo "‚ú?cine.md installed"
```

```powershell
# Windows
$base = Join-Path $env:USERPROFILE '.config\opencode'
if (Test-Path (Join-Path $base 'skills\cineprompt\SKILL.md')) { Write-Host "‚ú?SKILL.md installed" -ForegroundColor Green }
if (Test-Path (Join-Path $base 'commands\cine.md')) { Write-Host "‚ú?cine.md installed" -ForegroundColor Green }
```
